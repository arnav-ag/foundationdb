...
public:
	...
	// Credential management — looks for "token" field in credential files.
	// Skips S3's SDK auth and delegates to the base class file-based credential refresh.
	Future<Void> updateSecret() override;
	bool extractCredentialFields(JSONDoc& account) override;

	// URL includes provider=gcs parameter
